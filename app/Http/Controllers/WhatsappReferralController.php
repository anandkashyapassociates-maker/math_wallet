...
class WhatsappReferralController extends Controller
{
    ...
    public function processReferral(Request $request)
    {
        ...
        try {
            DB::transaction(function () use ($member, $memberid, $cleanMobile, $messageModel, $rewardAmount) {
                $today = now()->toDateString();
                ...
                // Update Pepe Wallet
                $member->pepe_wallet = ($member->pepe_wallet ?? 0) + $rewardAmount;
                $member->save();
        ...
        $waNum = ltrim($cleanMobile, '+');
        $encodedText = rawurlencode($finalMessageText);
        $whatsappUrl = "https://wa.me/{$waNum}?text={$encodedText}";

        $member->refresh();

        return response()->json([
            'success' => true,
            'message' => 'Referral processed! 1000 PEPE Tokens credited to your wallet.',
            'whatsapp_url' => $whatsappUrl,
            'pepe_wallet' => $member->pepe_wallet,
            'reward_amount' => $rewardAmount,
        ]);
    }
    ...
    public function referralDetails(Request $request)
    {
        ...
        $todayStatus = $waTodayCount > 0 ? 'Used' : 'Not Used';
